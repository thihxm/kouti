...
//  Kouti
//
//  Created by Marco Zulian on 30/06/21.
//
...
struct CellButtonView: View {
    ...
    var body: some View {
        ...
        case .edit:
            NavigationLink(
                destination: EditMissionView(task: task, isNewMission: false),
                label: {
...
extension View {
    func addButtonActions(task: TaskModel, leadingButtons: [CellButtons], trailingButton: [CellButtons], onClick: @escaping (CellButtons) -> Void) -> some View {
        self.modifier(SwipeContainerCell(task: task, leadingButtons: leadingButtons, trailingButton: trailingButton, onClick: onClick))
    }
}
...
struct SwipeContainerCell: ViewModifier  {
    ...
    init(task: TaskModel, leadingButtons: [CellButtons], trailingButton: [CellButtons], onClick: @escaping (CellButtons) -> Void) {
                ...
                HStack(spacing: 0) {
                    ForEach(trailingButton) { buttonsData in
                        Button(action: {
                            ...
                            CellButtonView.init(task: task, data: buttonsData, cellHeight: proxy.size.height)
                        })
                    }
...
//        TaskList(userManager: UserManager.fullState())
//    }
//}
